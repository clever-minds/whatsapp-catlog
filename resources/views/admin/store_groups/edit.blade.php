...
@section('content')
<div class="mb-6">
    <h1 class="text-3xl font-bold">Edit Store Group</h1>
</div>

<div class="bg-white shadow rounded-lg p-6 max-w-3xl">
    <form action="{{ route('store-groups.update', $storeGroup) }}" method="POST">
        @csrf
        @method('PUT')
        
        <div class="mb-4">
            <label class="block text-gray-700 font-bold mb-2">Group Name *</label>
            <input type="text" name="name" value="{{ $storeGroup->name }}" class="w-full border rounded px-3 py-2" required>
        </div>

        <div class="mb-6">
            <label class="block text-gray-700 font-bold mb-2">Select Stores to Include</label>
            @php $selectedStoreIds = $storeGroup->stores->pluck('id')->toArray(); @endphp
            <select name="stores[]" id="storeSelect" multiple required class="w-full border-gray-300 rounded-lg shadow-sm focus:border-blue-500 focus:ring-blue-500">
                @foreach($stores as $store)
                    <option value="{{ $store->id }}" {{ in_array($store->id, $selectedStoreIds) ? 'selected' : '' }}>
                        {{ $store->name }} ({{ $store->mobile_number }})
                    </option>
                @endforeach
            </select>
        </div>

        <button type="submit" class="bg-blue-600 text-white px-4 py-2 rounded hover:bg-blue-700">Update Group</button>
